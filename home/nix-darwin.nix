{
  ...
  # Nix command functions for easier usage
  programs.zsh.initExtra = ''
    ...
    hm-build() {
      nix build .#homeConfigurations.$(whoami)@$(hostname | cut -d'.' -f1).activationPackage
    }
    
    hm-check() {
      nix build .#homeConfigurations.$(whoami)@$(hostname | cut -d'.' -f1).activationPackage --dry-run
    }
    ...
    darwin-build() {
      nix build .#darwinConfigurations.$(hostname | cut -d'.' -f1).system
    }
    
    darwin-check() {
      nix build .#darwinConfigurations.$(hostname | cut -d'.' -f1).system --dry-run
    }
