...
# This file is sourced by zsh.nix

# ──────────────────────────────────────────────────────────────────
# PATH Configuration
# ──────────────────────────────────────────────────────────────────
export PATH="$HOME/.rd/bin:$PATH"  # Rancher Desktop uses .rd in HOME
...
generate-ssh-key() {
  ...
  local ssh_dir="${HOME}/.ssh"  # SSH uses ~/.ssh by convention, not XDG
  if [ -f "$ssh_dir/id_ed25519" ]; then
    ...
    case $choice in
      ...
      2)
        ...
        # Backup old key
        cp "$ssh_dir/id_ed25519" "$ssh_dir/id_ed25519.backup.$(date +%Y%m%d_%H%M%S)"
        cp "$ssh_dir/id_ed25519.pub" "$ssh_dir/id_ed25519.pub.backup.$(date +%Y%m%d_%H%M%S)"
        ;;
      3)
...
remove-aws-profile() {
  ...
  if [[ -f "$docker_config" ]]; then
    ...
    dependent_registries=$(jq -r --arg helper "$ecr_helper_name" '.credHelpers // {} | to_entries[] | select(.value == $helper) | .key' "$docker_config" 2>/dev/null)
